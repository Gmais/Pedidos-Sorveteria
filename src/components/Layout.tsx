...
export function Layout() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100">
      ...
      <nav className="fixed bottom-0 inset-x-0 z-20 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 sm:static sm:border-t-0 sm:bg-transparent sm:dark:bg-transparent">
        <div className="max-w-5xl mx-auto grid grid-cols-5 sm:flex sm:justify-center sm:gap-2 sm:py-3">
          {NAV_ITEMS.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
              ...
              className={({ isActive }) =>
                `flex flex-col sm:flex-row items-center justify-center gap-0.5 sm:gap-2 py-2 sm:px-4 sm:py-2 sm:rounded-lg text-xs sm:text-sm font-medium transition-colors ${
                  isActive
                    ...
                    : 'text-slate-500 dark:text-slate-400 sm:hover:bg-slate-100 sm:dark:hover:bg-slate-700'
                }`
              }
            >
